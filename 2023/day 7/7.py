...
def cmp_hands_enhanced(a, b):
    ...
    if len(cnt_a) > 1 and len(cnt_b) > 1:
        jkr_a = cnt_a.pop("J", 0)
        jkr_b = cnt_b.pop("J", 0)
        cnt_a[max(cnt_a, key=cnt_a.get)] += jkr_a
        cnt_b[max(cnt_b, key=cnt_b.get)] += jkr_b
...
def cmp(
    cards: str, hand_a: list[str], hand_b: list[str], cnt_a: list[int], cnt_b: list[int]
):
    if cnt_a != cnt_b:
        if max(cnt_a, cnt_b) == cnt_a:
            return 1
